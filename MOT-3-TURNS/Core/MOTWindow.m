classdef MOTWindow
    ...
    methods
        ...
        function DisplayMessageAndWaitRandom(obj, message, colour, wrapat)
            if nargin < 4
                wrapat = 50;
            end
            if nargin < 3
            ...
            obj.DisplayMessage(message, colour, wrapat);
            SitNWaitRandom(obj.WinHandle);
        end
        
        function DisplayMessageAndWaitShort(obj, message, colour, wrapat)
            if nargin < 4
                wrapat = 50;
            end
            if nargin < 3
            ...
            obj.DisplayMessage(message, colour, wrapat);
            SitNWaitShort(obj.WinHandle);
        end
        
        function DisplayMessageAndWaitMed(obj, message, colour, wrapat)
            if nargin < 4
                wrapat = 50;
            end
            if nargin < 3
            ...
            obj.DisplayMessage(message, colour, wrapat);
            SitNWaitMed(obj.WinHandle);
        end
        
        function autopresswait(obj)
            ...
            rob=Robot;
            rob.keyPress(java.awt.event.KeyEvent.VK_Y);
        end
        ...
        function [yes, time] = GetYNAutomatic(obj)
            time = -1;
            doLoop=1;
            yNum = KbName('y');
            nNum = KbName('n');
...
%%%%%%%%%%%
% SitNWaitRandom - wait for random secs
%%%%%%%%%%%
function SitNWaitRandom(win)
        waittime = round(rand(1)*3);
        WaitSecs(waittime);      
end

%%%%%%%%%%%
% SitNWait - wait for 35 secs
%%%%%%%%%%%
function SitNWait(win)
        WaitSecs(3);      
end

function SitNWaitMed(win)
        WaitSecs(5);      
end


%%%%%%%%%%%
% SitNWait - wait for 1 sec
%%%%%%%%%%%
function SitNWaitShort(win)
        WaitSecs(.6);      
end
...
%%%%%%%%%%%
% SitNWaitForTime - wait for a specificed amount of time before progressing
%%%%%%%%%%%
function SitNWaitForTime(win)
    WaitSecs(2);
end
